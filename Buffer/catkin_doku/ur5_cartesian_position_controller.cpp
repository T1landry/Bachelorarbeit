...
#include <cstdlib>					//hinzugefügt
#include "std_msgs/String.h"
#include "std_msgs/Float64MultiArray.h" //hinzugefügt
#include "std_msgs/Float64.h" 
#include "control_msgs/JointControllerState.h"
#include <iostream>					//hinzugefügt
#include <sstream>					//hinzugefügt
#include <iomanip>					//hinzugefügt
#include <fstream>					//hinzugefügt
#include <algorithm>				//hinzugefügt
...
int main(int argc, char **argv)
{
	...
	ros::Publisher joint_com_pub[7]; 	//die orinal Datei hat 6, aber wir haben einen unabhängige Topic hinzugefügt, der alle 6 position gleizeizig bekommt, die wir dann nutzen um unseren Zwilling zu aktualisieren 
	...
	joint_com_pub[6] = n.advertise<std_msgs::Float64>("/wrist_test_joint_position_controller/command", 1000);
	ros::Publisher pub = n.advertise<std_msgs::Float64MultiArray>("wrist_all_joint_position_controller/command", 1000); 	//dieser Topic bekommt die 6 Werte der 6 axen der Roboter auf einmal nicht nacheinander.

	// erstellung des hono-client Objekts

	//std::unique_ptr<PahoMqtt::Client> client(new PahoMqtt::Client("tcp://10.101.86.238:1883"));			// HONO_ADAPTER_IP: hier der Mqtt_adapter_IP von Hono, eingegeben.

	// initialisierung der Callback Funktionen:

	//client->setConnectedCallback(onConnected);
    //client->setDisconnectedCallback(onDisconnected);
    //client->setMessageCallback(onMessageReceived);

	//verbindung zu der HONO adapter:

	//MqttConnectOptions options;
	//options.setUsername("2dadfd97-cfec-4e1f-b1aa-ff589a9ba2ea@67c80785-fb4f-4f4b-89af-89c0f9da4d56");
	//options.setPassword("tchamabelandry6.");
	//client->connect(options);

	//warten, dass die verbindung ergestellt wird.

	//while (!client->isConnected()) {
        //std::this_thread::sleep_for(std::chrono::milliseconds(100));
    //}

	//jetzt abonieren wir das Topic wo die nachtricht auf Hono geschickt wird
	...
	while (ros::ok()) {
		...
		while(t<t_max) {
			...
			std::string data_string ;		//hinzugefügt
			std_msgs::Float64MultiArray msg; 	//hinzugefügt
			std_msgs::Float64 position[6];		//hinzugefügt
			//Compute next position step for all joints
			for(int i=0; i<Joints; i++) {
				position[i].data = compute_linear(jnt_pos_start(i), jnt_pos_goal(i), t, t_max);
				//msg.data = compute_linear(jnt_pos_start(i), jnt_pos_goal(i), t, t_max);
				msg.data.push_back(compute_linear(jnt_pos_start(i), jnt_pos_goal(i), t, t_max));  //hinzugefügt
				// speichern der Position fuer spätere nützung
				//save_position[i] = position[i].data;
				joint_com_pub[i].publish(position[i]);		
			}
			

			std::stringstream ss;						//hinzugefügt

			for (int i = 0; i < msg.data.size(); i++) {		//hinzugefügt
        		ss << msg.data[i] << " ";
    		}		
    		//ss << msg;
   			std::string array_string = ss.str();		//hinzugefügt


			std::ofstream file("value.txt");			//hinzugefügt
			if(file.is_open()){							//hinzugefügt
			
				file << array_string << std::endl;
				file.close();
			}

			 int decimal_places = 0; // Anzahl der Dezimalstellen
    		 for (auto value : msg.data) {
        
        		std::string value_str = std::to_string(value);
        // find the number of decimal places in each value
        		auto pos = value_str.find(".");				//hinzugefügt
        		if (pos != std::string::npos) {				// hinzugefügt
            		decimal_places = std::max(decimal_places, (int)(value_str.size()-pos-1));
        		}
    		}

			for (auto data : msg.data) {				//hinzugefügt
        		data_string += std::to_string(data);
    			}
			std_msgs::String str_msg;		//hinzugefügt
			str_msg.data = data_string;		//hinzugefügt
